...
- 网站
description: ''
summary: 这是一篇建立个人博客网站的指南，它将展示在Windows系统下，如何脱离繁琐的代码编写，仅通过开源网站框架（Hugo）与第三方平台（GitHub、Netlify、Forestry）之间的简单连接，在最低零成本的情况下，生成独属于个人的网站。  主要的思路如下：
draft: true
...
_本次，我在E盘新建了一个Hugo文件夹，将压缩包中的内容解压到了"E:\\Hugo"。_

#### 配置环境变量

1. 右键点击"我的电脑"，点击“属性”

![Hugo配置环境变量](file://F:/md/img/Hugo%E9%85%8D%E7%BD%AE%E7%8E%AF%E5%A2%83%E5%8F%98%E9%87%8F.jpg?lastModify=1581760264)
...
![hugo验证-1](file://F:/md/img/hugo%E9%AA%8C%E8%AF%81-1.png?lastModify=1581760264)

在命令行工具中输入：

    hugo version

![hugo验证-2](file://F:/md/img/hugo%E9%AA%8C%E8%AF%81-2.png?lastModify=1581760264)
...
2. 登录GitHub，并在[这里](https://github.com/new)创建新的仓库（repository），仓库类型设置为公开（public），点击create repository。
   ...
   ![](file://F:/md/img/create%20repository.png?lastModify=1581760264)
3. 创建完成后，我们需要复制下这个仓库的地址信息，即图中的红色部分，在之后将要用到。

   ![](file://F:/md/img/create%20repository%20done.png?lastModify=1581760264)
...
![](file://F:/md/img/git.png?lastModify=1581760264)

2. 之后，我们需要在git中存储我们的GitHub账号信息，以实现同步，我们需要在命令行输入我们上一步创建GitHub时，所用的用户名及邮箱信息：

       git config --global user.name "Your Name" //输入时不需要输入""，完成后回车git config --global user.email "email@example.com" //输入时不需要输入""，完成后回车
...
2. 新页面会出现最新报价，如果出现的是外国货币，可以点击页面底部，选择到"新加坡-简体中文"，这样不仅会解决语言和货币单位的问题，还可以支持支付宝付款。如果筛选到了满意的域名，就可以加入购物车并结算。
   ...
   ![](file://F:/md/img/godaddy%20%20sg.png?lastModify=1581760264)
...
5. 付款。然后，这个域名就属于你了。你可以说随时在你的GoDaddy页面内查看它。
   ...
   ![](file://F:/md/img/%E5%9F%9F%E5%90%8D%E8%AE%BE%E7%BD%AE.png?lastModify=1581760264)
